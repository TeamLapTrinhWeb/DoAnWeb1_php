		...
		<div class="tab-pane  active" id="blockView">
			<ul class="thumbnails">
				<?php
					...
					$id = $_GET["id"];
					$c_sql = "select count(*) as num_rows from sanpham where maNSX = $id";
					$c_rs = load($c_sql);
					...
					$sql = "select * from sanpham where maNSX = $id limit $offset, $limit ";
					$rs = load($sql);
					if ($rs->num_rows > 0) :
	...
	<div class="pagination">
		<ul>
			<?php for ($i = 1; $i <= $num_pages; $i++) : ?>	
				<li><a href="TrangProducts.php?id=<?php echo"$id"; ?>&page= <?php echo"$i" ?>"><?php echo"$i"; ?></a></li>
			<?php endfor; ?>
		</ul>
	</div>
	<br class="clr"/>
